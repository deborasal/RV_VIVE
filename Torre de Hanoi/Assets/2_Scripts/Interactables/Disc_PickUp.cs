...
public class Disc_PickUp : Pickup_Touch {
    ...
    {
        initialPos = transform.position;
        return base.CanBeManipulated(other) && GetInteractionDistance(other) < grabRange && manager.GetComponent<disk_manage>().TryPop(transform.GetComponent<properties>().GetSize(), initialPos);
    }
